...
public class AmazonInterviewQ2: Solution {
    ...
    public func execute() -> Int? {
        ...
        var left = 0
        var right = 0
        var previousLeftValue: Character = "1"
        var previousRightValue: Character = "1"
        var zeroCount = 0
        ...
        while right < servers.count {
            ...
            print(
                "[\(left)...\(right)]: \(servers[left..<right]), consecutiveOnServers: \(right - left) zeroCount: \(zeroCount), maxConsecutiveOnServers: \(maxConsecutiveOnServers)"
            )
        }
